...
describe WidgetApi::VotesController do
  ...
  describe 'voting on a idea' do
    example 'casting an up-vote' do
      post(:cast, value: 2, token: @idea.application.token, idea_id: @idea.id, user_email: @user.email, format: :json)
    ...
    example 'cancel an upvote' do
      ...
      post(:cast,value: 2, token: @idea.application.token, idea_id: @idea.id, user_email: vote.voter.email, format: :json)
      expect(Vote.count).to eq(1)
  ...
  describe 'voting on a comment' do
    example 'casting a vote' do
      ...
      post(:cast, value: 2, token: comment.idea.application.token, comment_id: comment.id, user_email: @user.email, format: :json)
      expect(Vote.count).to eq(1)
      expect(Vote.first.value).to eq(1)
    ...
    example 'changing a vote' do
      ...
      post(:cast, value: 2, token: comment.idea.application.token, comment_id: comment.id, user_email: vote.voter.email, format: :json)
      expect(Vote.count).to eq(1)
      expect(Vote.first.value).to eq(0)
    end
  end
