  ...
  context 'normal notification' do
    describe 'notify_group' do
      it 'should create notifications' do
        ...
        expect(Notification).to receive(:create_with).with(action, subject, user, app).twice
        n = NotificationCreator.new(action, subject, app)
  ...
  context 'attribute notification' do
    describe 'notify_group' do
      ...
      it 'should create notifications, even if receiver equals action.creator' do
        ...
        n = NotificationCreator.new(action, subject, app, action_attribute, action_attribute_changer)
        n.notify_group([user])
      end
      ...
      it 'should not create notification for the user who changed the attribute and triggered notification' do
        ...
        n = NotificationCreator.new(action, subject, app, action_attribute ,user)
        n.notify_group([user])
      end
    end
  end
